...
##
##SEGMENTED_OUTPUT_DIR = './extractedContours/'
...
if os.path.exists('./outputImages/.DS_Store'):
    os.remove('./outputImages/.DS_Store')

segmented_images = []
files = [f for r, d, f in os.walk(SEGMENTED_OUTPUT_DIR)][0]
files.sort(key=lambda x: int(x.strip('.jpg')))
...
import re
parsed_str = re.sub('L', '', parsed_str)

# parsed_str = parsed_str[::-1]
# parsed_str.strip('L')

##with open('stringResult.txt', 'w') as out:
